...
use crate::fs::aliases::{
    code_for_name, is_corporate_suffix, is_place_code, name_for_code, state_code_for_name,
    state_name_for_code,
};
use crate::fs::bytes::*;
use crate::fs::facts::{best_agreement, fact_multiplier};
use crate::fs::profile::{profile, Profile};
use crate::fs::sets::{Set, EMPTY_SET};
use crate::fs::tokens::{mark_boilerplate, tokenize, Toks, EMPTY_TOKS, K_NUMBER};
use crate::fs::units::annotate_units;
...
fn add_country_aliases(set: &mut Set, t: &Toks) {
    ...
    while i < t.n {
        ...
        if t.kind[i] == crate::fs::tokens::K_WORD {
            if t.proper[i] {
...
fn polarity_of(ta: &Toks, tg: &Toks, p: &Profile) -> f32 {
    ...
    // pairs. Treat it as what it is -- a contradiction of a stated finding.
    {
        let mut a = 0usize;
        while a < ta.n {
            if !ta.boiler[a] && !ta.echo[a] && crate::fs::antonyms::is_verdict(ta.hash[a]) {
                let mut g = 0usize;
                let (mut stated_same, mut stated_opposite) = (false, false);
                while g < tg.n {
                    if tg.hash[g] == ta.hash[a] {
                        stated_same = true;
                    } else if crate::fs::antonyms::opposes(ta.hash[a], tg.hash[g]) {

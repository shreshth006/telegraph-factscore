...
use crate::fs::bytes::*;
use crate::fs::profile::Profile;
use crate::fs::sets::Set;
use crate::fs::tokens::{Toks, K_IDENT, K_NUMBER};
use crate::fs::units::*;
...
mod tests {
    ...
    fn a_wrong_figure_multiplies_down_but_not_to_a_cliff() {
        ...
        let mut sr = crate::fs::sets::EMPTY_SET;
        sr.fill(&right);
        let (mr, _) = fact_multiplier(&right, &tg, &sr, &p);
        let mut sw = crate::fs::sets::EMPTY_SET;
        sw.fill(&wrong);
    ...
    fn unasserted_facts_are_neutral() {
        ...
        let mut sa = crate::fs::sets::EMPTY_SET;
        sa.fill(&ta);
